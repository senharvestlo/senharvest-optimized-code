...
export default function ProformaHtmlEditor({ docId=null, onBack }) {
  ...
  const [state, setState] = useState({
    ...
    notes: ['This is a proforma invoice; not a tax invoice. Subject to final contract.'],
  ...
  useEffect(()=>{ (async()=>{
    ...
    if (d){
      if (d.data) setState({ ...d.data, type:'proforma' });
      else setState(prev=>({ ...prev, ...(d || {}), type:'proforma' }));
    }
    setLoading(false);
  })(); }, [docId]);
  ...
  return (
    <>
      ...
      <div className="p-4 md:p-6">
        ...
        {/* Rendu imprimable */}
        <div ref={ref} className="max-w-5xl mx-auto bg-white border rounded-lg p-6 md:p-8 text-sm" id="proforma">
          {/* Header */}
                  ...
                  return (
                    <tr key={idx} className="align-top">
                      <td className="border px-2 py-2" contentEditable suppressContentEditableWarning onBlur={e=>{
                        const v = e.currentTarget.textContent; setField(`items.${idx}.description`, v);
                      }}>{it.description}</td>
                      <td className="border px-2 py-2" contentEditable suppressContentEditableWarning onBlur={e=>{
                        const v = e.currentTarget.textContent; setField(`items.${idx}.grade`, v);
                      }}>{it.grade}</td>
                      <td className="border px-2 py-2 text-center" contentEditable suppressContentEditableWarning onBlur={e=>{
                        const v = Number(e.currentTarget.textContent)||0; setField(`items.${idx}.quantity`, v);
                      }}>{it.quantity}</td>
                      <td className="border px-2 py-2 text-center" contentEditable suppressContentEditableWarning onBlur={e=>{
                        const v = e.currentTarget.textContent; setField(`items.${idx}.unit`, v);
                      }}>{it.unit}</td>
                      <td className="border px-2 py-2 text-center" contentEditable suppressContentEditableWarning onBlur={e=>{
                        const v = e.currentTarget.textContent; setField(`items.${idx}.pack`, v);
                      }}>{it.pack}</td>
                      <td className="border px-2 py-2 text-center" contentEditable suppressContentEditableWarning onBlur={e=>{
                        const v = e.currentTarget.textContent; setField(`items.${idx}.hsCode`, v);
                      }}>{it.hsCode}</td>
                      <td className="border px-2 py-2 text-center" contentEditable suppressContentEditableWarning onBlur={e=>{
                        const v = Number(e.currentTarget.textContent)||0; setField(`items.${idx}.unitPrice`, v);
          ...
          <div className="rounded border p-4 mb-6 avoid-break-inside">
            ...
            <div className="border-t mt-3 pt-3 grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              <p><strong>Bank:</strong> <span contentEditable suppressContentEditableWarning onBlur={e=>setField('company.bank.name', e.currentTarget.textContent)}>{state.company.bank.name}</span></p>
              <p><strong>Beneficiary:</strong> <span contentEditable suppressContentEditableWarning onBlur={e=>setField('company.bank.beneficiary', e.currentTarget.textContent)}>{state.company.bank.beneficiary}</span></p>
              <p><strong>IBAN/ACC:</strong> <span contentEditable suppressContentEditableWarning onBlur={e=>setField('company.bank.iban', e.currentTarget.textContent)}>{state.company.bank.iban}</span></p>
              <p><strong>SWIFT/BIC:</strong> <span contentEditable suppressContentEditableWarning onBlur={e=>setField('company.bank.swift', e.currentTarget.textContent)}>{state.company.bank.swift}</span></p>
            </div>
          </div>
          ...
          <div className="mt-6 flex justify-between avoid-break-inside pb-safe">
            ...
            <div>
              ...
              <p className="mt-6 border-t w-48"></p>
              <p contentEditable suppressContentEditableWarning onBlur={e=>setField('buyerSign.companyName', e.currentTarget.textContent)}>{state.buyerSign.companyName}</p>
            </div>
          </div>
        </div>
      </div>
