...
export default function QuotationHTML() {
  ...
  }, [limitTwoPages, lang, currency, number, date, seller, buyer, lines, terms]);

  const handleDownload = async () => {
    let scaled = 1;
    if (limitTwoPages) scaled = fitToPages(shellRef.current, fitRef.current, 2);
    await downloadElementAsPDF(shellRef.current, `Quotation-${number||date}.pdf`);
  };
        ...
        <div className="pdf-fit-wrap">
          <div className="pdf-sheet" ref={fitRef}>
            <div className="header">
              <div>
                <div className="badge">QUOTATION / DEVIS</div>
                <h1 className="h1 mt-2">{T('Devis / Quotation','Quotation')}</h1>
